...
func Clone(url, branch, commit_hash, path string) error {
	...
	if err != nil {
		...
		LOGGER.Warnf("SSH authentication will be disabled, falling back to HTTPS")
		use_ssh = false
	} else {
	...
	if use_ssh && ssh_key_path != "" {
		...
		if strings.HasPrefix(url, "https://github.com/") {
			...
			clone_url = ssh_url
			LOGGER.Debugf("Converted HTTPS URL to SSH: %s", clone_url)
		...
		LOGGER.Debugf("Using SSH authentication with key: %s", ssh_key_path)
	} else {
		// Cloning with HTTPS (SSH not available) - operational info, no user message needed
	}
	...
	LOGGER.Debugf("Executing: git %s", strings.Join(cloneArgs, " "))
	...
	if commit_hash != "" {
		LOGGER.Debugf("Checking out commit: %s", commit_hash)
		...
		if checkoutErr != nil {
			...
			LOGGER.Debugf("Git checkout output: %s", string(checkoutOutput))
			return fmt.Errorf("git checkout failed: %v", checkoutErr)
		}

		fmt.Println("Git checkout successful")
	}
...
func Pull(repo_path string) error {
	...
	if is_ssh_repo && ssh_key_path != "" {
		...
		// Using HTTP authentication (no SSH configuration needed) - operational info
	}

	// Execute git pull command
	LOGGER.Debugf("Executing: git pull --ff-only")
...
func Push(repo_path string) error {
	...
	LOGGER.Debugf("Executing: git add -A")
	...
	if addErr != nil {
		...
		LOGGER.Debugf("Git add output: %s", string(addOutput))
		return fmt.Errorf("git add failed: %v", addErr)
	}

	// Check if there are changes to commit
	LOGGER.Debugf("Executing: git status --porcelain")
	...
	if statusErr != nil {
		...
		LOGGER.Debugf("Git status output: %s", string(statusOutput))
	...
	// Execute git commit
	LOGGER.Debugf("Executing: git commit -m '%s'", commit_message)
	...
	if commitErr != nil {
		...
		LOGGER.Debugf("Git commit output: %s", string(commitOutput))
		return fmt.Errorf("git commit failed: %v", commitErr)
	}

	// Execute git push
	LOGGER.Debugf("Executing: git push origin HEAD")
	...
	if pushErr != nil {
		fmt.Printf("Git push failed for the following reason: %v\n", pushErr)
	...
	fmt.Println("Git push successful")
	LOGGER.Debugf("Git push output: %s", string(pushOutput))
